#!/usr/bin/env node
// WaveSpeed text-to-image CLI, MAX-QUALITY variant of generate-image.mjs.
...
// the top tier is the slowest to render. You can still pass --quality /
// --resolution to dial it back down for a given run.
//
// Usage:
...
const POLL_TIMEOUT_S = 600; // top tier (high/4k) is slow; give it room
...
async function main() {
  ...
  if (!opts.prompt) {
    console.error(
      'Usage: npm run gen:max -- "<prompt>"  [--quality=low|medium|high]' +
