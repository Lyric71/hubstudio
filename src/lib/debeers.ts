/**
 * Shared server logic for the hidden DeBeers review page (/debeers).
 *
 * Two concerns, no dependencies beyond the platform:
 *   - Auth: a password gate backed by an HMAC-signed, httpOnly session cookie
 *     (Web Crypto). The password is never stored in the cookie; the cookie is
 *     just a signed, expiring token the server can verify.
 *   - Comments: stored in Vercel KV (Upstash Redis) over its REST API with
 *     plain fetch, one Redis list per asset. No client library needed.
 *
 * Configure in the Vercel project (or .env for local dev):
 *   DEBEERS_PASSWORD         the shared access password (falls back to the
 *                            documented default if unset).
 *   DEBEERS_SESSION_SECRET   secret used to sign session cookies. SET THIS in
...
const PASSWORD = import.meta.env.DEBEERS_PASSWORD ?? 'hubstudio888+';
const SECRET =
  import.meta.env.DEBEERS_SESSION_SECRET ?? 'debeers-fallback-secret-change-me-2026';

const COOKIE_NAME = 'debeers_session';
...
function key(assetId: string): string {
  return `debeers:comments:${assetId}`;
}
